...
# MongoDB initialization wrapped safely
try:
...
# 2. Flask Setup integrated with event routing
flask_app = Flask(__name__)

@flask_app.route('/')
def index():
    return "Bot Engine Status: Operational", 200

@flask_app.route('/webhook', methods=['POST'])
def webhook():
    # Fetch current running event loop safely to process the queue
    loop = asyncio.get_event_loop()
    update_data = request.get_json(force=True)
    update = Update.de_json(update_data, bot_app.bot)
    
    # Process updates inside the existing main event loop cleanly
    loop.create_task(bot_app.process_update(update))
    return "OK", 200

async def main_engine():
    # Single unified startup loop execution
    await bot_app.initialize()
    
    if RENDER_EXTERNAL_URL:
        webhook_url = f"{RENDER_EXTERNAL_URL.rstrip('/')}/webhook"
        await bot_app.bot.set_webhook(url=webhook_url)
        print(f"✅ Webhook active on: {webhook_url}")
    else:
        print("⚠️ Running in local/polling engine backup.")
        await bot_app.start()
        await bot_app.updater.start_polling(drop_pending_updates=True)

    # Booting up Flask production web service inside the same standard asyncio engine
    ...
    print("🚀 Unified Hypercorn Server starting...")
